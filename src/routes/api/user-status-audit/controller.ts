...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await Models.UserStatusAudit.findAll();
		res.json(data);
	} catch (e) {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.UserStatusAudit.findByPk(req.params.id);
...
router.post("/", validateBody(Models.UserStatusAudit as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await Models.UserStatusAudit.create(req.body);

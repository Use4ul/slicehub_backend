...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await Models.ModelComment.findAll();
		res.json(data);
	} catch (e) {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.ModelComment.findByPk(req.params.id);
...
router.post("/", validateBody(Models.ModelComment as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await Models.ModelComment.create(req.body);

...
import { modelTagService } from "../../../services";

const router = Router();

router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await modelTagService.findAll();
		res.json(data);
	} catch (e) {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await modelTagService.findById(req.params.id);
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		const deleted = await modelTagService.delete(req.params.id);

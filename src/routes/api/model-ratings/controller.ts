...
import { modelRatingService } from "../../../services";

const router = Router();

router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await modelRatingService.findAll();
		res.json(data);
	} catch (e) {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await modelRatingService.findById(req.params.id);
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		const deleted = await modelRatingService.delete(req.params.id);

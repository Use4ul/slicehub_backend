...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await Models.ModelRating.findAll();
		res.json(data);
	} catch (e) {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.ModelRating.findByPk(req.params.id);
...
router.post("/", validateBody(Models.ModelRating as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await Models.ModelRating.create(req.body);

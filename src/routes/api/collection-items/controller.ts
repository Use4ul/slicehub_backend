...
import { collectionItemService } from "../../../services";
...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await collectionItemService.findAll();
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await collectionItemService.findById(req.params.id);
...
router.post("/", async (req: Request, res: Response) => {
	try {
		const created = await collectionItemService.create(req.body);
...
router.put("/:id", async (req: Request, res: Response) => {
	try {
		const updated = await collectionItemService.update(req.params.id, req.body);
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		const deleted = await collectionItemService.delete(req.params.id);

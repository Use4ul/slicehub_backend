...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await Models.CollectionItem.findAll();
		res.json(data);
	} catch {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.CollectionItem.findByPk(req.params.id);
...
router.post("/", async (req: Request, res: Response) => {
	try {
		const created = await Models.CollectionItem.create(req.body);

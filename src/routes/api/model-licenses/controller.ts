...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await Models.ModelLicense.findAll();
		res.json(data);
	} catch {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.ModelLicense.findByPk(req.params.id);
...
router.post("/", validateBody(Models.ModelLicense as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await Models.ModelLicense.create(req.body);

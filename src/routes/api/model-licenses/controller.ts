...
import { modelLicenseService } from "../../../services";
...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await modelLicenseService.findAll();
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await modelLicenseService.findById(req.params.id);
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		const deleted = await modelLicenseService.delete(req.params.id);

...
import { fileTypeService } from "../../../services";
...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await fileTypeService.findAll();
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await fileTypeService.findById(req.params.id);
...
router.post("/", validateBody(Models.FileType as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await fileTypeService.create(req.body);
...
router.put("/:id", validateBody(Models.FileType as any, { partial: true }), async (req: Request, res: Response) => {
	try {
		const updated = await fileTypeService.update(req.params.id, req.body);
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		const deleted = await fileTypeService.delete(req.params.id);

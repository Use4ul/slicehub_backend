...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await licenseService.findAll();
		res.json(data);
	} catch {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await licenseService.findById(req.params.id);
...
router.post("/", validateBody(Models.License as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await licenseService.create(req.body);
...
router.put("/:id", validateBody(Models.License as any, { partial: true }), async (req: Request, res: Response) => {
	try {
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		const deleted = await licenseService.delete(req.params.id);

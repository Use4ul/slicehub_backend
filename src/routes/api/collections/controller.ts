...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await Models.Collection.findAll();
		res.json(data);
	} catch {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.Collection.findByPk(req.params.id);
...
router.post("/", async (req: Request, res: Response) => {
	try {
		const created = await Models.Collection.create(req.body);

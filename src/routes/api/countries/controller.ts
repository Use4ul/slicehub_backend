...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.Country.findByPk(req.params.id);
...
router.post("/", validateBody(Models.Country as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await Models.Country.create(req.body);
...
router.put("/:id", validateBody(Models.Country as any, { partial: true }), async (req: Request, res: Response) => {
	try {
		const row = await Models.Country.findByPk(req.params.id);
		if (!row) return res.status(404).json({ message: E.NOT_FOUND });

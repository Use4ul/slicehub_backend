...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.City.findByPk(req.params.id);
...
router.post("/", validateBody(Models.City as any, { partial: false }), async (req: Request, res: Response) => {
	try {
		const created = await Models.City.create(req.body);
...
router.put("/:id", validateBody(Models.City as any, { partial: true }), async (req: Request, res: Response) => {
	try {
		const row = await Models.City.findByPk(req.params.id);
		if (!row) return res.status(404).json({ message: E.NOT_FOUND });

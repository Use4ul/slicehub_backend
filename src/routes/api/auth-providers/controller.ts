...
router.get("/", async (req: Request, res: Response) => {
	try {
		const data = await Models.AuthProvider.findAll();
		res.json(data);
	} catch {
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		const row = await Models.AuthProvider.findByPk(req.params.id);
...
router.post("/", async (req: Request, res: Response) => {
	try {
		const created = await Models.AuthProvider.create(req.body);

...
export class License extends Model<LicenseAttributes, LicenseCreationAttributes> implements LicenseAttributes {
  ...
  static initialize(sequelize: Sequelize) {
    return License.init({
      ...
      allows_commercial_use: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
      allows_modification: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
      requires_attribution: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: true },
  ...
  static associate(models: Models) {
    License.belongsToMany(models.Model3d, { through: models.ModelLicense, foreignKey: 'license_id', as: 'models' });
  }
}
